...
class NNmodel:
    ...
    def plot_Loss_Error_HR_Iso_in_one(self, grid, iso_ages, indices, isos, plot_MSE=True, extended=True, epochs=None, this_train=False, widths=None, N=5000, one_per_track=True, track_no=20, std_limit=True, savefile=None, trial_no=None):
        ...
        if type(epochs)!=type(None):
            ...
            MAE = MAE[epochs[0]:epochs[1]+1]
            valMAE = valMAE[epochs[0]:epochs[1]+1]
        ax[0,0].plot(epoch,MAE,'b',label='MAE')
        ax[0,0].plot(epoch,valMAE,'r',label='valMAE')
        if plot_MSE==True:
            if 'MSE' in keys:
                MSE,valMSE=hist['MSE'],hist['val_MSE']
            elif 'mae' in keys:
            ...
            if type(epochs)!=type(None):
                MSE = MSE[epochs[0]:epochs[1]+1]
                valMSE = valMSE[epochs[0]:epochs[1]+1]
            ax[0,0].plot(epoch,MSE,'b:',label='MSE')
            ax[0,0].plot(epoch,valMSE,'r:',label='valMSE')
        ...
        ax[1,0].scatter(Teffm,Lm,s=5,c=M, cmap='viridis')
        ax[1,0].set_xlim(ax[1,0].get_xlim()[::-1])
        ax[1,0].set_ylabel(r'$\log10(L/L_{\odot})$')
        ax[1,0].set_xlabel(r'$\log10 T_{eff}$')
        #ax[1,0].set_title('NN predicted')
        s2=ax[1,1].scatter(Teffg,Lg,s=5,c=M, cmap='viridis')
        ax[1,1].set_xlim(ax[1,1].get_xlim()[::-1])
        ax[1,1].set_ylabel(r'$\log10(L/L_{\odot})$')
        ax[1,1].set_xlabel(r'$\log10 T_{eff}$')
        ax[1,1].set_title('MESA data')
        #fig.subplots_adjust(right=0.83)
        cbar_ax = fig.add_axes([0.63, ax[1,1].get_position().y0, 0.02, ax[1,1].get_position().height])
        cbar_ax.text(0.5,1.015,r'$M/M_{\odot}$',fontsize=13,horizontalalignment='center',transform=cbar_ax.transAxes)
        fig.colorbar(s2, cax=cbar_ax)
        ##############################isochrone plot code
        if widths is None:
        ...
        [Teffm, Lm, Am, Teffg, Lg, Ag] = [NN_tracks[output_index.index('Teff')],
                                          NN_tracks[output_index.index('L')],
                                          NN_age, plot_data[0], plot_data[1], 10**plot_data[2]]
        ax[2,0].scatter(Teffm,Lm,s=5,c=Am, cmap='viridis')
        ax[2,0].set_xlim(ax[2,0].get_xlim()[::-1])
        ax[2,0].set_ylabel(r'$\log10(L/L_{\odot})$')
        ax[2,0].set_xlabel(r'$\log10 T_{eff}$')
        #ax[2,0].set_title('NN predicted')
        s2=ax[2,1].scatter(Teffg,Lg,s=5,c=Ag, cmap='viridis')
        ax[2,1].set_xlim(ax[2,1].get_xlim()[::-1])
        ax[2,1].set_ylabel(r'$\log10(L/L_{\odot})$')
        ax[2,1].set_xlabel(r'$\log10 T_{eff}$')
        ax[2,1].set_title('MESA data')
        ax[2,0].set_xlim(ax[2,1].get_xlim())
        ax[2,0].set_ylim(ax[2,1].get_ylim())
        #fig.subplots_adjust(right=0.83)
        cbar_ax = fig.add_axes([0.63, ax[2,1].get_position().y0, 0.020, ax[2,1].get_position().height])
        cbar_ax.text(0.5,1.015,'Age(Gyr)',fontsize=13,horizontalalignment='center',transform=cbar_ax.transAxes)
        fig.colorbar(s2, cax=cbar_ax)
        #######################################
        ...
        for i,Dout in enumerate(y_out):
            ...
            sns.distplot(errors, bins=200, ax=ax[3,i], label='median='+str(median))
            ax[3,i].set_xlabel(x_labels[i])
            ax[3,i].legend()
            if std_limit==True:
                ax[3,i].set_xlim([-10*std,10*std])
        ...
        plt.show()
        if savefile != None:
            fig.savefig(savefile+'/Combo'+str(trial_no)+'.png')
            print('Multigraph saved as "'+savefile+'/Combo'+str(trial_no)+'.png"')
