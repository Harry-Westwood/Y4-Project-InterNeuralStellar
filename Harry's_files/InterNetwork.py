...
def Leg_Setup(folder_name,reg,lr,epoch_no,batch_size,opt,activation,dropout):
    leg_num, list_params = Get_Num_Legs(reg=reg,lr=lr,epoch_no=epoch_no,batch_size=batch_size,opt=opt,activation=activation,dropout=dropout)
    legs = []
    for leg in range(leg_num):
        ...
        if type(lr) is not list:
            leg_dict['lr'] = lr
        if type(epoch_no) is not list:
            leg_dict['epoch_no'] = epoch_no
        if type(batch_size) is not list:
...
def NN_run(folder_name, df, contNN, changeingNNparam,hidden_layers,activation,reg,dropout,opt,lr,epoch_no,batch_size,leg):
    ...
    if (contNN == True and changeingNNparam == True) or (contNN == False and changeingNNparam == False):
        ...
        m1.compileModel(opt=opt, lr=lr, loss='MAE', metrics=['MAE','MSE'], beta_1=0.9995, beta_2=0.999) #
...
def NN_results(grid_file, folder_name,starttime,start_epoch,last_leg=False):
    ...
    m1.loadModel('drive/My Drive/4th Year Project/{}/{}_best_model.h5'.format(folder_name,folder_name), summary = False)
    m1.loadHist('drive/My Drive/4th Year Project/{}/Hist{}'.format(folder_name,folder_name),'pickle')
    final_epoch = m1.history['legs'][-1]['epoch_no']
    #
    ...
    m1.plotHist(plot_MSE=False, epochs=[start_epoch,final_epoch],this_train=False) 
    ...
    if last_leg == True:
        ...
        m1.plot_Loss_Error_HR_Iso_in_one(grid=grid1, iso_ages=np.linspace(1,11,12), extended=False, this_train = False, indices=['age','feh','Y','MLT'], isos=[0, 0.28,1.9], plot_MSE=False, epochs=None, widths=[['r',0.05],['a',0.1],['a',0.01],['a',0.1]], N=5000, one_per_track=False, track_no=100, savefile='drive/My Drive/4th Year Project/{}'.format(folder_name), trial_no=folder_name)
...
def Get_Num_Legs(reg,lr,epoch_no,batch_size,opt,activation,dropout):
    leg_num = -1
    list_params = [] #parameters that have leg variants so are in list form
    if reg != None:
        if type(reg) is list:
            if type(reg[0]) is list:
                if len(reg) > leg_num:
