...
def NN_run(folder_name, df, hidden_layers, legs, leg):
    ...
    if contNN == False:
        start_epoch = 0
        #from datetime import datetime
    x_cols = ['mass', 'age', 'tenstarfeh', 'Y', 'MLT'] #inputs cols
    y_cols = ['radius', 'scale_T', 'delnu']
    #if 'scale_radius' in df.columns:
    #    y_cols = ['scale_radius', 'scale_T', 'delnu'] # output cols
    #else:
    #    y_cols = ['radius', 'scale_T', 'delnu'] # output cols
...
def NN_results(grid_file, folder_name,starttime,start_epoch,training_track_no,in_between,radius_coeffs,last_leg=False):
    finishtime = datetime.now().strftime("%d-%m-%Y %H:%M:%S")
    if radius_coeffs != None:
        m1=NNmodel('evo',['mass', 'age', 'feh', 'Y', 'MLT'], ['radius','Teff', 'delnu'], Teff_scaling=5000,radius_coeffs=radius_coeffs)
    else:
    ...
    #
    m1.plotHist(plot_MSE=False, this_train=False)
    ...
    if last_leg == False:
        m1.plotHR(grid=grid1, track_no=20, in_between=in_between)
        #m1.plotIsochrone(grid1, np.linspace(1,11,12), ['age','feh','Y','MLT'], [0, 0.28,1.9], widths=[['r',0.05],['a',0.1],['a',0.01],['a',0.1]], one_per_track=False, savefile='drive/My Drive/4th Year Project/{}'.format(folder_name), trial_no=folder_name,extended=False)
        print(last_loss)
    ...
    if last_leg == True:
        ...
        if training_track_no is list:
            evaluation = m1.evalData(grid1, list(range(len(training_track_no))))
        else:
            evaluation = m1.evalData(grid1, [0])
        ...
        m1.plotHR(grid=grid1, track_no=20, in_between=in_between, savefile='drive/My Drive/4th Year Project/{}'.format(folder_name), trial_no=folder_name)
        #m1.plots_for_one_track(grid=grid1, in_between=in_between, plot_MSE=False, epochs=None, track_no=100, savefile='drive/My Drive/4th Year Project/{}'.format(folder_name), trial_no=folder_name)
        #m1.plot_Loss_Error_HR_Iso_in_one(grid=grid1, iso_ages=np.linspace(1,11,12), indices=['age','feh','Y','MLT'], isos=[0, 0.28,1.9], in_between=in_between, plot_MSE=False, epochs=None, widths=[['r',0.05],['a',0.1],['a',0.01],['a',0.1]], N=5000, one_per_track=False, track_no=100, savefile='drive/My Drive/4th Year Project/{}'.format(folder_name), trial_no=folder_name)
        for i in runresults:
            print(i)
