...
class stellarGrid:
    ...
    def initialData(self, initial_columns=None, base=10):
        """
        ...
        if 'track_no' not in self.data.columns:
            ...
            if initial_columns is None:
                if 'initial_feh' in self.data.columns:
                    initials = np.array(self.data[['initial_mass', self.proper_index[4], 'initial_feh', self.proper_index[5]]])
                else:
                    initials = np.array(self.data[['initial_mass', self.proper_index[4], self.proper_index[3], self.proper_index[5]]])
            else: initials = np.array(self.data[initial_columns])
        ...
        if 'frac_track_age' not in self.data.columns:
            ...
            for i in self.data['track_no'].unique():
                this_track = self.data[self.data['track_no']==i]
                self.data['track_max_age'][this_track.index] = max(this_track['age'])
            self.data['frac_track_age'] = base**(self.data['age']/self.data['track_max_age'])/base
...
class NNmodel:
    ...
    def plotIsochrone(self, grid, iso_ages, indices, isos, widths=None, N=5000, 
        ...
        if 'frac_track_age' in self.input_index:
            NN_age=10**NN_tracks[self.output_index.index('age')]
        else:
    ...
    def findFracAge(self, target_age, mass, fixed_inputs, indices, threashold):
        frac_age_lims = [0,1]
        age_guess = -1
        x_in = []
        count = 0
        ...
        for i,param in enumerate(self.input_index):
            if param == 'mass':
                x_in[i] = np.ones(10)*np.log10(mass)
            else:
                for j,ind in enumerate(indices):
                    if param==ind:
                        if param in self.non_log_columns:
                            x_in[i] = np.ones(10)*fixed_inputs[j]
                        else:
                            x_in[i] = np.ones(10)*np.log10(fixed_inputs[j])
        while abs(age_guess-target_age) > threashold:
            count+=1
            x_in_temp = x_in.copy()
            frac_age_list = np.linspace(*frac_age_lims,10)
            x_in_temp[self.input_index.index('frac_track_age')] = frac_age_list
            x_in_temp = np.array(x_in_temp)
            y_out = self.model.predict(x_in_temp.T).T
            age_results = 10**y_out[self.output_index.index('age')]
            abs_age_diff = abs(age_results-target_age)
            sort_order = np.argsort(abs_age_diff)
            age_guess = age_results[sort_order[0]]
            frac_age_guess = frac_age_list[sort_order[0]]
            frac_age_lims = np.sort([frac_age_guess,frac_age_list[sort_order[1]]])
            if count>20:
                print('Warning: Unable to find optimal frac track age for target age.')
                break
    ...
    def plotTrends(self, grid, track_no=20, in_between=None, nth_track=None, savefile=None, trial_no=None):
        ...
        if in_between is None:
            ...
            y_out = self.model.predict(x_in.T,verbose=2, batch_size=len(x_in.T)).T
            if 'frac_track_age' in self.input_index:
                Agem = 10**y_out[self.output_index.index('age')]
            else:
    ...
    def getDex(self, grid):
        ...
        dex_values = {}
        for i,Dout in enumerate(y_out):
            if self.output_index[i] in self.non_log_columns:
                Mout = NN_tracks[i]
            else:
                Mout = 10**NN_tracks[i]
                Dout = 10**Dout
    ...
    def plotOffTracks(self, df, ax, moved_name, moved_index, title, labels, cutoff=False):
        first = True
        if 'radius' in self.output_index:
            lum_index = self.output_index.index('radius')
        else: lum_index = self.output_index.index('L')
        Teff_index = self.output_index.index('Teff')
        for track_no in df['track_no'].unique():
            this_track = df[df['track_no']==track_no].copy()
            this_track.sort_values(by='age', inplace=True)
            on_in = np.array(this_track[self.input_index]).T
            on_out = self.predict(on_in)
            if first == True:
                ax.plot(on_out[Teff_index], on_out[lum_index], c='black', zorder=3, label='tracks')
            else:
                ax.plot(on_out[Teff_index], on_out[lum_index], c='black', zorder=3)
            first = False
        lims = [ax.get_xlim(),ax.get_ylim()]
    
        moved_input_names = self.input_index.copy()
...
def data_calFracAge(df, age_col, base=10):
    if 'frac_track_age' not in df.columns:
        if 'track_no' not in df.columns:
            raise KeyError('Tracks have not been divided yet!! Use function data_divideTracks first.')
        df['track_max_age'] = np.zeros(len(df.index))
        for i in df['track_no'].unique():
            this_track = df[df['track_no']==i]
            df['track_max_age'][this_track.index] = max(this_track[age_col])
